...
export default function Home() {
  ...
  const [paceFilter, setPaceFilter] = useState<'Unsolved' | 'Solved' | 'All'>('Unsolved');
  ...
  // --- FILTERING ---
  const patternFilteredQuestions = questions.filter(q => 
    patternFilter === 'All' || q.pattern === patternFilter
  );
      ...
      <aside className="w-64 flex flex-col p-4 gap-4 bg-white border-r border-gray-200">
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
           <div className="p-3 bg-gray-50 border-b border-gray-100 rounded-t-xl">
            <h2 className="text-xs font-bold text-gray-500 uppercase tracking-wider">Patterns</h2>
          </div>
          <div className="overflow-y-auto flex-1 p-2 space-y-1 custom-scrollbar border-x border-b border-gray-100 rounded-b-xl">
            <button
              ...
              className={`w-full text-left px-3 py-2 rounded-md text-sm font-medium transition-all ${
                patternFilter === 'All' ? 'bg-gray-900 text-white' : 'text-gray-600 hover:bg-gray-100'
              }`}
            >
              All Patterns
            </button>
            ...
            {LEARNING_PATH.map(p => (
              <button
                ...
                className={`w-full text-left px-3 py-2 rounded-md text-sm transition-all ${
                  patternFilter === p ? 'bg-blue-600 text-white shadow-md' : 'text-gray-600 hover:bg-gray-100'
                }`}
              >
                {p}
              </button>
          ...
          <div className="flex-1 overflow-y-auto pr-2 space-y-3 pb-20">
            {paceList.map(q => (
              <QuestionCard 
                key={q.id} 
                {...q} 
                // In Pace, Green = Solved
                isSolved={!!solvedLog[q.id]} 
                onToggle={toggleProblem} 
              />
            ))}
             {paceList.length === 0 && <div className="text-center text-gray-400 mt-10 text-sm">No problems found.</div>}
          </div>
        </section>
        ...
        <section className="flex-1 flex flex-col min-w-0 border-l border-gray-200 pl-6 border-dashed">
          <div className="mb-4 flex justify-between items-center">
            <div>
               ...
               <p className="text-xs text-amber-600/70 font-medium">Review due items</p>
            </div>
            ...
            <div className="flex gap-2">
              {[3, 7, 30].map((day) => (
                <button
                  key={day}
                  onClick={() => setRepaceFilter(repaceFilter === day ? 'All' : day as any)}
          ...
          <div className="flex-1 overflow-y-auto pr-2 space-y-3 pb-20">
            ...
            {repaceList.length === 0 && (
              <div className="text-center mt-10 p-6 bg-amber-50/50 rounded-xl border border-amber-100 border-dashed">
                <p className="text-amber-900/40 text-sm font-medium">You are all caught up!</p>
              </div>
            )}
          </div>
        </section>
